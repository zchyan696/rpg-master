...
export default function NovaSessao() {
  ...
  const [form, setForm] = useState({ number: '', title: '', date: today, summary: '', notes: '' })
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input style={fieldStyle} type="number" min="1" value={form.number} onChange={(e) => set('number', e.target.value)} required placeholder="1" className="mt-1" />
            </div>
            <div className="col-span-2">
              <label style={labelStyle}>Título *</label>
              <input style={fieldStyle} value={form.title} onChange={(e) => set('title', e.target.value)} required placeholder="Ex: A Chegada à Cidade" className="mt-1" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <textarea style={{ ...fieldStyle, resize: 'vertical', minHeight: '100px' }} value={form.summary} onChange={(e) => set('summary', e.target.value)} placeholder="O que aconteceu nessa sessão? Breve resumo para lembrar depois..." className="mt-1" />
          </div>
          <div>
            <label style={labelStyle}>Notas do Mestre</label>
            <textarea style={{ ...fieldStyle, resize: 'vertical', minHeight: '120px' }} value={form.notes} onChange={(e) => set('notes', e.target.value)} placeholder="Anotações detalhadas, NPCs introduzidos, pistas reveladas, o que funcionou ou não..." className="mt-1" />
          </div>
          <div className="flex items-center gap-3 pt-2">
            <button type="submit" disabled={loading || !form.number || !form.title} className="flex items-center gap-2 px-6 py-2.5 rounded-sm cursor-pointer disabled:opacity-50" style={{ background: '#1a2a4a', color: '#F5F0E8', fontFamily: "'Crimson Text', serif", fontSize: '1rem', border: '1px solid #2a3a6a' }}>
              <Save size={15} /> {loading ? 'Salvando...' : 'Salvar Sessão'}
            </button>
            <Link href="/sessoes">
