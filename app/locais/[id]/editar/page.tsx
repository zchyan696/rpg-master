...
export default function EditarLocal() {
  ...
  useEffect(() => {
    fetch(`/api/locations/${id}`).then((r) => r.json()).then((data) => {
      setForm({ name: data.name || '', category: data.category || 'other', description: data.description || '', notes: data.notes || '' })
    })
  }, [id])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) router.push(`/locais/${id}`)
    } finally {
      setLoading(false)
    }
  }
    ...
    <PageTransition>
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(180deg, #0a1a0a 0%, #0f0f0f 100%)', minHeight: '140px' }}>
        <div className="relative z-10 px-10 pt-8 pb-6">
          ...
          <h1 className="text-3xl font-bold" style={{ fontFamily: "'Playfair Display', serif", color: '#F5F0E8' }}>Editar Local</h1>
        </div>
        <ZigzagDivider />
      </div>
      <div className="px-10 py-8 max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea style={{ ...fieldStyle, resize: 'vertical', minHeight: '100px' }} value={form.description} onChange={(e) => set('description', e.target.value)} className="mt-1" />
          </div>
          <div>
